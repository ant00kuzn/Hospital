...
{
    public partial class PatientStatus : Form
    {
        private int id = -1;
        ...
        private void LoadData()
        {
            try
            {
                ...
                connection.Open();
                MySqlCommand cmd = new MySqlCommand(@"select statusID, statusName as 'Статус' from patientstatus order by statusName desc", connection);
                MySqlDataAdapter adapter = new MySqlDataAdapter(cmd);
                DataTable table = new DataTable();

                adapter.Fill(table);

                dataGridViewStatus.DataSource = table;
        ...
        private void dataGridViewStatus_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                {
                    dataGridViewStatus.Rows[e.RowIndex].Selected = true;
                    id = Convert.ToInt32(dataGridViewStatus.Rows[e.RowIndex].Cells[0].Value.ToString());
                    textBox1.Text = dataGridViewStatus.Rows[e.RowIndex].Cells[1].Value.ToString();
                    prevStatusName = textBox1.Text;
                    prevRowIndex = e.RowIndex;
                    textBox1.Focus();
                }
                else
                {
                    dataGridViewStatus.Rows[e.RowIndex].Selected = false;
                    id = -1;
                    textBox1.Text = "";
                    prevRowIndex = -1;
                    dataGridViewStatus.Focus();
                }
            }
            else
            {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (textBox1.Text != "")
            {
                if (textBox1.Text.Length > 4)
                {
                    try
                    {
                        ...
                        MySqlCommand checkCmd = new MySqlCommand($"SELECT COUNT(*) FROM patientstatus WHERE statusName = '{statusName}'", connection);

                        if ((long)checkCmd.ExecuteScalar() > 0)
                        {
                            MessageBox.Show("Статус с таким наименованием уже существует.", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            textBox1.Focus();
                            connection.Close();
                            return;
                        }

                        MySqlCommand cmd = new MySqlCommand($"insert into patientstatus (statusName) value ('{statusName}')", connection);
                        cmd.ExecuteNonQuery();
                ...
                {
                    MessageBox.Show("Наименование статуса должно быть не короче 5 символов.", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    textBox1.Focus();
                    return;
                }
            }
            ...
            {
                MessageBox.Show("Заполните обязательное поле с наименованием статуса.", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                textBox1.Focus();
                return;
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                if (textBox1.Text.Length > 4 && prevStatusName != textBox1.Text)
                {
                    try
                    {
                        string statusName = textBox1.Text.Trim();
                        ...
                        MySqlCommand cmd = new MySqlCommand($"update patientstatus set statusName = '{statusName}' where statusID = {id}", connection);
                        cmd.ExecuteNonQuery();
                ...
                {
                    MessageBox.Show("Наименование статуса должно быть изменено и не короче 5 символов.", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    textBox1.Focus();
                    return;
                }
            }
            ...
            if (id != -1)
            {
                if (DialogResult.Yes == MessageBox.Show("Вы уверены, что хотите удалить запись?", "Подтверждение", MessageBoxButtons.YesNo, MessageBoxIcon.Stop))
                {
                    try
                    {
                        MySqlConnection connection = new MySqlConnection(GlobalValue.GetConnString());
                        connection.Open();
                        MySqlCommand cmd = new MySqlCommand($"delete from patientstatus where statusID = {id}", connection);
                        cmd.ExecuteNonQuery();
